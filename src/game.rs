...
impl EventHandler for Game {
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        if self.paused {
            let cars_in_intersection = Text::new(format!("Max numbers vehicles: {}", self.cars_amount));


            let cars_max_time = Text::new(format!("max time: {} seconds", self.max_time));
            let cars_min_time = Text::new(format!("min time: {} seconds", self.min_time));

            let cars_top_speed = Text::new(format!("max velocity: {}km/h", self.top_speed));
            let cars_min_speed = Text::new(format!("min velocity: {}km/h", self.min_speed));

            let cars_close_calls = Text::new(format!("close calls: {}", self.close_calls));
            let cars_collision = Text::new(format!("collisions: {}", self.close_calls));
...
impl Assets {
    pub fn new(ctx: &mut Context) -> Self {
        Self {
            ...
            _car_black: Image::from_path(ctx, "/car.png").unwrap(),
            car_blue: Image::from_path(ctx, "/car.png").unwrap(),
            car_green: Image::from_path(ctx, "/car.png").unwrap(),
            car_red: Image::from_path(ctx, "/car.png").unwrap(),
            car_yellow: Image::from_path(ctx, "/car.png").unwrap(),
        }
    }
}